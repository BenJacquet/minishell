...
/*   Updated: 2020/10/28 17:01:01 by chgilber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*finddolar(t_all *all, int *i, int j, char **tmp)
{
	...
	//all->dol[all->igno] = (tmp[2][0] == '\0') ? -666 : all->dol[all->igno][0];
...
{
	all->dol[all->igno][0] = i;
	tmp[3] = finddolar(all, &i, j, tmp);
	all->dol[all->igno][1] =  ft_strlen(tmp[3]);
	printf(" start = %d end = %d , de [%s]\n", all->dol[all->igno][0], all->dol[all->igno][1], tmp[3]);
	all->igno++;
	free(tmp[0]);
	free(tmp[2]);
	all->j = 0;
